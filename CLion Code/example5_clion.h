...
#ifndef UNTITLED_EXAMPLE5_CLION_H
#define UNTITLED_EXAMPLE5_CLION_H
...
public:
    int longestArrow(std::string s){
        ...
        for(int i = 0; i < slength; i++){
            ...
            }else if(s.at(i) == '<' && i+1 < slength &&s.at(i+1) == '-'){
                int j = i+1;
                arrowLength++;
        ...
        int max = -1;
        for(int arrowLength : arrowLengths){
            if(arrowLength > max){
                max = arrowLength;
            }
        }
...
#endif //UNTITLED_EXAMPLE5_CLION_H
