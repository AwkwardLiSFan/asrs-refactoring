...
#ifndef UNTITLED_EXAMPLE11_CLION_H
#define UNTITLED_EXAMPLE11_CLION_H
#include <iostream>
#include <string>
#include <vector>
using namespace std;
...
public:
    int howMany(string beforeTime, string afterTime) {
        ...
        if (vec_BT.at(3) > 9) {     // Checks if the ones in the minute section is more than one digit
            vec_BT.at(2) ++;        // Increments the tens section and resets the ones section
            vec_BT.at(3) = 0;
        }

        if (vec_BT.at(2) == 6) {    // if the minutes reach 60, increment hours by 1 and change minutes back to 00
            vec_BT.at(2) = 0;
            vec_BT.at(1) ++;
        }

        if (vec_BT.at(1) > 9) {     // if the ones in the hour section is more than one,
            vec_BT.at(0) ++;        // increments the tens section and resets the ones section
            vec_BT.at(1) = 0;
        }

        if (vec_BT.at(0) == 2 && vec_BT.at(1) == 4) {   // If the hours reach 24:00, sets it to 00:00
            vec_BT.at(0) = 0;
            vec_BT.at(1) = 0;
        }
...
#endif //UNTITLED_EXAMPLE11_CLION_H
